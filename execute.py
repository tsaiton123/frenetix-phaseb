...
def execute():
    ...
    for idx, scenario in enumerate(scenario_files):
        ...
        while not done:
            ...
            if done:
                # Visualization is non-essential; protect the outer scenario loop
                # from any per-scenario plotting failure (e.g. pandas concat /
                # schema mismatches in agent_run_visualization).
                try:
                    visualize_agent_run(test_env.simulation.log_path, mod_path)
                except Exception as e:
                    print(f"  (visualize_agent_run skipped: {e})")
