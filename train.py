...
def main():
    ...
    # Create a list of callbacks (TqdmCallback shows a progress bar that renders
    # cleanly in Colab/Jupyter and as a text bar in a terminal)
    total_timesteps = training_configs["total_timesteps"]
    callbacks = CallbackList([
        TensorboardCallback(env_configs),
    ...
    # progress_bar=False: TqdmCallback handles the bar (SB3's built-in rich bar
    # renders poorly in Colab).
    model.learn(total_timesteps=total_timesteps, callback=callbacks, tb_log_name='PPO', progress_bar=False)
