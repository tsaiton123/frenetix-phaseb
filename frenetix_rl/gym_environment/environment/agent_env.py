...
class AgentEnv(gym.Env):
    def __init__(self, scenario_paths,
        ...
        # Phase B: when action_type == "weights_and_sampling" the agent additionally
        # outputs 2 sampling actions (longitudinal horizon + lateral half-span) that
        # dynamically shape the Frenet sampling grid. Otherwise (action_type ==
        # "weights") the action is prediction-weight + cost-weight updates only.
        _acfg = self.general_configs["action_configs"]
        self._use_sampling_action = _acfg["action_type"] == "weights_and_sampling"
        n_action = 1 + len(_acfg["cost_terms"]) + (2 if self._use_sampling_action else 0)
        self.action_space = sh.construct_action_space(n_action)
        ...
        if plot_agents:
            self.config_sim.visualization.save_all_individual_plots = True
        ...
        # Phase B: dynamic sampling-grid action parameters.
        if self._use_sampling_action:
            # Longitudinal horizon is sampled in [sample_t_low, planning_horizon];
            # the planner's fixed t_min (< sample_t_low) stays the lower bound of
            # the time-sampling range. Lateral half-span dd in [sample_d_low,
            # sample_d_high] -> grid uses delta_d in [-dd, +dd]. Small dd + large
            # horizon = "long & narrow"; large dd + small horizon = "short & wide".
            self.planner_t_min = self.config_planner.planning.t_min
            self.sample_t_low = action_configs["sample_t_low"]
            self.sample_t_high = self.config_planner.planning.planning_horizon
            assert self.planner_t_min < self.sample_t_low <= self.sample_t_high, \
                "Require planner t_min < sample_t_low <= planning_horizon"
            self.sample_d_low = action_configs["sample_d_low"]
            self.sample_d_high = action_configs["sample_d_high"]
            self._set_rescale_sample_factors_and_biases()
    ...
    def set_action(self, action):
        ...
        weights = action[1:1 + len(self.cost_terms)]
        ...
        self._apply_sampling_action(action)
    ...
    def _apply_sampling_action(self, action):
        """Phase B: apply the (already-rescaled) sampling actions to the planner.

        action[1+n_cost] = longitudinal horizon (s); action[2+n_cost] = lateral
        half-span dd (m). No-op when action_type != "weights_and_sampling".
        """
        if not self._use_sampling_action:
            return
        n_cost = len(self.cost_terms)
        horizon = float(np.clip(action[1 + n_cost], self.sample_t_low, self.sample_t_high))
        dd = float(np.clip(action[2 + n_cost], self.sample_d_low, self.sample_d_high))
        self.agent.planner_interface.planner.set_sampling_parameters(
            t_min=self.planner_t_min, horizon=horizon, delta_d_min=-dd, delta_d_max=dd)
    ...
    def rescale_action(self, action: np.ndarray):
        ...
        n_cost = len(self.cost_terms)
        # prediction cost weight (cumulative update)
        ...
        # cost weights (cumulative updates)
        action[1:1 + n_cost] = action[1:1 + n_cost] * self._rescale_weight_factor + self._rescale_weight_bias

        # Phase B sampling actions are ABSOLUTE grid bounds (not cumulative):
        # horizon then lateral half-span, mapped from [-1, 1] to their real ranges.
        if self._use_sampling_action:
            action[1 + n_cost] = action[1 + n_cost] * self._rescale_horizon_factor + self._rescale_horizon_bias
            action[2 + n_cost] = action[2 + n_cost] * self._rescale_dd_factor + self._rescale_dd_bias
        return action
    ...
    def _set_rescale_sample_factors_and_biases(self):
        # Maps normalized [-1, 1] -> [low, high] for horizon and lateral half-span.
        self._rescale_horizon_factor = (self.sample_t_high - self.sample_t_low) / 2.
        self._rescale_horizon_bias = self.sample_t_high - self._rescale_horizon_factor
        self._rescale_dd_factor = (self.sample_d_high - self.sample_d_low) / 2.
        self._rescale_dd_bias = self.sample_d_high - self._rescale_dd_factor
