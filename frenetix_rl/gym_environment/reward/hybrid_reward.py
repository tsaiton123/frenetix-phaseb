...
class HybridReward(Reward):
    ...
    def calc_reward(self, observation_dict: OrderedDict, action: np.ndarray, agent, config_sim=None, simulation=None,
        ...
        # Phase B: reward for using a smaller sampling grid (fewer candidates).
        if self.reward_configs.get("reward_sampling_efficiency"):
            rewards.append(self._sampling_efficiency_reward())
        else:
            rewards.append(0)
    ...
    def _sampling_efficiency_reward(self) -> float:
        """Penalty proportional to the number of candidate trajectories sampled
        this step (i.e. the size of the Frenet grid the RL agent shaped)."""
        trajectories = self.agent.all_trajectories
        n_traj = len(trajectories) if trajectories else 0
        return self.reward_configs["reward_sampling_efficiency"] * n_traj
